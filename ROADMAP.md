...
4. Run complet del RAW amb cascada (decisió 2026-08-05): `cpegen titles` (dedup + filtre de soroll dels exports SCCM) → `cpegen run --resume` amb `qwen3-1.7b` (passada ràpida, escriptura incremental per fila) → `cpegen escalate --model qwen3-8b` (re-run de la cua no-M1x + merge amb traça `escalated_by`/`fast_rule`). Tooling fet i testejat (2026-08-05); pendent el run real al PC
